...
class OfflineTaskWithRealEvaluations(Task):
    def __init__(self, buffer_size: int, env_name: str):
        ...
        """
        _, self.eval_env, self.train_dataset, self.val_dataset = make_env_and_datasets(env_name)
    ...
    def sample(self, dataset: Literal['train', 'val'], batch_size: int):
        return self.train_dataset.sample(batch_size) if dataset == 'train' else self.val_dataset.sample(batch_size)
