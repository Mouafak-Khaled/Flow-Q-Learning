...
class Decoder(nn.Module):
    ...
    @nn.compact
    def __call__(self, z):
        z = nn.relu(nn.Dense(self.hidden_size)(z))
...
class InitialObservationEnvModel(nn.Module):
    ...
    def setup(self):
        self.encoder = Encoder(self.latent_dimension, self.hidden_size)
        self.decoder = Decoder(self.observation_dimension, self.hidden_size)
